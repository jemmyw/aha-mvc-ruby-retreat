 ...
 * 
 * Here is the todo item, and this is not so bad.
 * 
 ...
 * it. 
 * 
 * Remember that neat Suspense feature? I've actually implemented that for MVC
 * and decided it adds more complexity that it's worth. For the gain of a
 * cascading load you get the additional advantage of needing to debug a thrown
 * promise causing rerender loops. My conclusion is, save Suspense for the
 * purpose it was designed for - code loading.
 * 
 * Our add button does have more code. But in my opinion this is correct view
 * code - dealing with class names. However, we do not have to deal with the
 * state of the todo items in order to instruct the controller to add one.
 * 
 * Our item view is also very similar, the only difference here is that it calls
 * controller actions instead of mutating the state itself.
 * 
 ...
 * state.
 * 
 * The actions are emminently more readable in my opinion, when compared to the
 * code flow through the Recoil example. The add action, it sets a flag for the
 * view to say we're creating, it adds a blank item to the list, it saves the
 * list, it unsets the flag.
 * 
